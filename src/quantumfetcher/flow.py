...
class DownloadFlow:
    ...
    def __init__(self, videoList, manifests, output_path) -> None:
    ...
    def download(self, fetch_episodes, fetch_qualities):
        with Live(self.__progress_group):
            ...
            for episode in fetch_episodes:
                self.__progress_overall.update(
                    progress_overall, advance=0, description=f"Downloading {episode}..."
                )
    ...
    def __download_episode(self, episode_id, qualities):
        ...
        for stream_type in media_to_download:
            self.__download_streams(
                episode_id, episode_path, stream_type, media_to_download[stream_type]
            )

    def __download_streams(self, episode_id, episode_path, type, streams):
        progress_stream = self.__progress_stream.add_task(
            f"Downloading {type.name} streams...", total=len(streams)
        )
        ...
        for stream, chunks in streams:
            ...
            self.__progress_stream.update(progress_stream, advance=1)
